...
struct PresentationSection: View {
    ...
    var body: some View {
        SettingsCard(title: "Presentation Mode", systemImage: "cursorarrow.rays") {
            Toggle("Keep a highlight around the pointer", isOn: $settings.isEnabled)

            Text("Toggle anytime with \(HotKeyManager.shortcutDescription).")
                .font(.callout)
                .foregroundStyle(.secondary)

            DisclosureGroup("Appearance and click effects", isExpanded: $isExpanded) {
